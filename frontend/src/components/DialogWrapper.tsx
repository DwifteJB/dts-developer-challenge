...
export default function DialogWrapper({
  ...
  return createPortal(
    <div
      className={`fixed inset-0 flex items-center justify-center transition-all ${open ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
      {/* fake background */}
      <div
        onClick={() => {
      ...
      />
      <div
        className={`bg-white text-black border border-[#2D2D2D] rounded-lg p-4 w-96 z-20 transition-all transform ${open ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}
      >
        {children}
      </div>
    </div>,
